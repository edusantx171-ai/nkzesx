...
app.post('/cadastrar', (req, res) => {
  ...
  const { nome, emails, pedidos_recentes, carrinho, cartao, assinaturas, dispositivos, endereco, vale, extra } = req.body;
  const data = new Date().toISOString().split('T')[0];
  db.run('INSERT INTO clientes (nome, emails, pedidos_recentes, carrinho, cartao, assinaturas, dispositivos, endereco, vale, extra, data_cadastro) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    [nome, emails, pedidos_recentes, carrinho, cartao, assinaturas, dispositivos, endereco, parseFloat(vale) || 0, extra, data], (err) => {
      if (err) {
...
app.put('/clientes/:id', (req, res) => {
  ...
  db.run('UPDATE clientes SET nome = ?, emails = ?, pedidos_recentes = ?, carrinho = ?, cartao = ?, assinaturas = ?, dispositivos = ?, endereco = ?, vale = ?, extra = ? WHERE id = ?',
    [nome, emails, pedidos_recentes, carrinho, cartao, assinaturas, dispositivos, endereco, parseFloat(vale) || 0, extra, id], (err) => {
      if (err) {
